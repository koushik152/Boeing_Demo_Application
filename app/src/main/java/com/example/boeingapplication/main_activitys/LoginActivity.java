...
import com.example.boeingapplication.loginactivities.CommonService;
import com.example.boeingapplication.loginactivities.LoginRequest;
import com.example.boeingapplication.loginactivities.LoginResponse;
import com.example.boeingapplication.loginactivities.LogoutRequest;
import com.example.boeingapplication.loginactivities.LogoutResponse;
import com.example.boeingapplication.loginactivities.ProfileDetailRequest;
import com.example.boeingapplication.loginactivities.ProfileDetailResponse;
        ...
        call.enqueue(new Callback<LoginResponse>() {
            ...
            public void onResponse(Call<LoginResponse> call, Response<LoginResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    String token = responseBody.getKey();
                    String  jsessionid=responseBody.getJSESSIONID();
                    SharedPreferences.Editor editor = sharedPreferences.edit();
                    editor.putString(TOKEN_KEY, token);
                    editor.putString(JSESSIONID_KEY,jsessionid);
                    editor.apply();
            ...
            call.enqueue(new Callback<ProfileDetailResponse>() {
                ...
                public void onResponse(Call<ProfileDetailResponse> call, Response<ProfileDetailResponse> response) {
                    if (response.isSuccessful() && response.body() != null) {
                        ...
                        String displayText = "Profile Details: " + profileDetailResponse.getAccountInfo().getDefaultOrgId();
                        Response.setText(displayText);
                    } else {
                        try {
                            String errorBody = response.errorBody().string();
                            Response.setText("Request failed:hii " + errorBody);
                        } catch (Exception e) {
                            e.printStackTrace();
                            Response.setText("Request failed: Unable to parse error response");
                        }
                ...
                public void onFailure(Call<ProfileDetailResponse> call, Throwable t) {
                    t.printStackTrace();
                    Response.setText("Request failed: jai " + t.getMessage());
                }
            });
        } else {
            Response.setText("Token not found in SharedPreferences");
        }
    }
